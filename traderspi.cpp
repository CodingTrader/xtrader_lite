...
void CtpTdSpi::UpdatePosByTd(LPVOID pParam,const CThostFtdcTradeField *pTrade)
{
	...
	double dLastAvg = 0.0,dClosePro=0.0,dfRByM,dfRByV,dFee=0;
	...
	for(vpos;vpos!=m_InvPosVec.end();vpos++)
	{
			...
			(pTrade->OffsetFlag!=THOST_FTDC_OF_Open))
		{
			...
			//上次均价
			dLastAvg = vpos->PositionCost/(iMul*vpos->Position);
			...
			{ GetLastTdVol(&(*vpos),iTdDelta,cFlag); }	
			...
			vpos->PositionCost = GetCurOpenCost(&(*vpos),iMul);
			GetInstMgr(pTrade->InstrumentID,vpos->PosiDirection,vpos->MarginRateByMoney,vpos->MarginRateByVolume);
			vpos->UseMargin = (vpos->PositionCost*vpos->MarginRateByMoney+vpos->Position*vpos->MarginRateByVolume);
			//逐笔平仓盈亏
			dClosePro = iFlag*(pTrade->Price-dLastAvg)*iMul*pTrade->Volume;
...
//今仓按FIFO计算 昨仓按结算价计成本
double CtpTdSpi::GetCurOpenCost(const CThostFtdcInvestorPositionField* pInv,int iMul)
{
	double dCost=0.0;
...
//只能在OnRtnTrade里调用保证tdvec是最新,未被修改
//Qry到的持仓记录里昨仓的数量恒定为当天的初始值!!
//最后一笔成交里的今仓昨仓数量iTd iYd 
int CtpTdSpi::GetLastTdVol(const CThostFtdcInvestorPositionField* pInv,int& iTd,TThostFtdcOffsetFlagType& cFlag)
{
	iTd=0; //iYd=0;
	int iSum=0,iPreSum=0,iLastVol=0,iYdPos=pInv->YdPosition;
	//昨仓为0 时简化
	if (iYdPos <1)
	{
		int iSize = static_cast<int>(m_tradeVec.size());
		iTd = (iSize>=1)?m_tradeVec[iSize-1].trade.Volume:0;
		cFlag = THOST_FTDC_OF_CloseToday;
		return 0;
	}

	TThostFtdcDirectionType  tDir = THOST_FTDC_D_Buy+THOST_FTDC_PD_Short- pInv->PosiDirection;
	for (VIT_td vtd=m_tradeVec.begin();vtd != m_tradeVec.end();vtd++)
	{
		if((!strcmp(vtd->trade.InstrumentID,pInv->InstrumentID)) && (vtd->trade.OffsetFlag!=THOST_FTDC_OF_Open)
			&& (vtd->trade.Direction==tDir))
		{
			iLastVol = vtd->trade.Volume;
			iSum += iLastVol;
		}
	}
	iPreSum = iSum-iLastVol;
